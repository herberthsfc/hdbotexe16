	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝙏𝙘𝙝𝙖𝙪 𝙘𝙤𝙧𝙣𝙤(𝙖) @${num.split('@')[0]} 🐂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[ ❗ ]  Este comando só funciona em grupos!',
					ownerB: '[ 💎 ]  Comando exclusivo para o Herberth!',
					admin: '[ 💎 ]  Comando exclusivo para usuários Premium ou Adms! Seu membro Comum!',
					Badmin: '[ ❗ ]  Este comando só funciona quando o bot se torna admnistrador!',
		...
		client.sendMessage(from, nsfwmenu(prefix, sender), text, {quoted: mek})
				...
				case 'rebaixar':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('[ ❗ ]  Marque o adm que deseja rebaixar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `[ ✔️ ]  Pedido aceito, passe para o administrador do grupo! :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`[ ❗ ]  𝙍𝙚𝙗𝙖𝙞𝙭𝙖𝙙𝙤(𝙖)! @${mentioned[0].split('@')[0]}\n Foi rebaixado(a) para membro(a) comum!${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'jshsh':
                  ...
                  case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`[ 💎 ]  𝙋𝙧𝙤𝙢𝙤𝙫𝙞𝙙𝙤(𝙖)! : @${mentioned[0].split('@')[0]} Foi Promovido(a) a ADM do grupo!`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
					case 'hentai2': 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'dono':
               ...
               client.sendMessage(from, '[ 💎 ]  Este é o número do meu criador. por favor, Salve!',MessageType.text, { quoted: mek} )
				break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                /*if (!isDaftar) return reply(mess.only.daftarB)*/
...
break
			    case 'liberar':
					...
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('[ ❗ ]  Hentai ja esta ativado!')
						nsfw.push(from)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('[ ❗ ]  Hentai Ativado')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('[ ❗ ]  Hentai Desativado')
					} else {
		...
		case 'spamcall':
			...
			case 'mapa': 
				...
				case 'ytbaixa':
				  if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('[ ❗ ]   Qual é link?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, '[ ❗ ]  Digite o código de idioma!', text, {quoted: mek})
                                   /*if (!isDaftar) return reply(mess.only.daftarB)*/
				...
				case 'imagens':
						...
						nimek =  n[Math.floor(Math.random() * n.length)];
						pok = await getBuffer(nimek)
						client.sendMessage(from, pok, image, { quoted: mek, caption: `[ ❗ ]  Imagem aleatória : *${body.slice(11)}*`})
						break
				case 'pinterest':
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.fdci.se/rep.php?gambar=${body.slice(11)}`, {method: 'get'})
				...
				case 'prefixo':
					...
					prefix = args[0]
					reply(`[ 💎 ]  O Prefixo foi alterado com sucesso  : ${prefix}`)
					break
				case 'mekgme':
						   ...
						   if (text.includes("random"))
						   {
							...
							axios.get(url)
							  .then((result) => {
								...
								.then(
									(response) => {
							conn.sendMessage(id, '[ ❗ ]  Carregando...', MessageType.text, { quoted: m } )
				...
				case 'ts':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*[ 💎 ]  Transmissão HDBOT.exe*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*[ 💎 ]  Transmissão para o*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*[ 💎 ]  Transmissão para o*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `[ ❗ ]  Quem criou o grupo , foi o : @${from.split("-")[0]} Mas quem manda somos nós!`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('[ ❗ ]  Digite o número!')
					if (args[0].startsWith('08')) return reply('[ ❗ ]  Digite o código do país!')
					try {
				...
				case 'ban':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('[ ❗ ]  Marque o alvo que voce quer remover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'HAHAHA, Foi removido, trouxa!', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'bug':
				client.updatePresence(from, Presence.composing) 
				if (args.length < 1) return reply('[ ❗ ]  Digite o bug')
					tek = body.slice(10)
					bug = {
	    ...
	    case 'kapankah':
			...
			case 'fechargp':
					...
					const close = {
					text: `[ ❗ ]  Grupo fechado pelo adm @${nomor.split("@s.whatsapp.net")[0]}\nagora apenas admnistradores podem enviar mensagens!`,
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					if (!isQuotedVideo) return reply('[ ❗ ]  Erro!')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('[ ❗ ]  Não foi possivel converter o video em mp3!')
						buffer = fs.readFileSync(ran)
            ...
            break
				...
				/*if (isDaftar) return reply(mess.only.daftarB)*/
					hobby = body.slice(1)
					const hob =['Sim','Não','Não sei','Lindo(a)','Gostosa','Vadia','Sei la','Que?','Foda-se','Mia Khalifa','Não quero falar com bandeirantes','Fdp','Dahora','Pesquisa no google','Princesa','Pinto pequeno','Virgem','Legal','Corno(a)']
					const by = hob[Math.floor(Math.random() * hob.length)]
					client.sendMessage(from, 'Sua pergunta : *'+hobby+'*\n\nBOT diz : '+ by, text, { quoted: mek })
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
				...
				default:
                  ...
                  reply(`[ ❗ ]  Não entendi o que quis dizer com isso! Para acessar meus comandos, digite ${prefix}$comandos`)
                                        }
